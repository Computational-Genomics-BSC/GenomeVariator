...
def _stats_from_alignment(alignment_file):
    """
    Calculate average read size from an alignment file.
    """
    f = pysam.AlignmentFile(alignment_file, 'rb')
...
def variate_alignment(args):
    ...
    original_stats = _stats_from_alignment(args.input_alignment)
    ...
    for i, command_tuple in enumerate(command_list):
        ...
        replace_reads(mutated_alignment, modified_reads_bam_file,
                      new_mutated_alignment, exclude_reads_file, args.processes)
